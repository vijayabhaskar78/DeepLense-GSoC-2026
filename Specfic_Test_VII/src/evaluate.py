"""Evaluation script for PINNClassifier (Specific Test VII — Physics-Guided ML).
...
Outputs (plots/)
----------------
    roc_curves_standard/tta.png
    confusion_matrix_standard/tta.png
    training_curves.png
    confidence_analysis.png
    ring_examples.png   -- grid of 9 images with predicted ring overlays
"""
...
WEIGHTS_PATH = os.path.join(_ROOT, "weights", "best_pinn.pth")
...
def plot_roc(labels, probs, tag=""):
    ...
    title = "PINN Classifier — ROC Curves" + (f" ({tag})" if tag else "")
...
def plot_confusion(labels, preds, tag=""):
    ...
    for ax, data, fmt, sfx in zip(
        ...
        ax.set(xlabel="Predicted", ylabel="True", title=f"Confusion Matrix — {sfx}")
        ...
        for r in range(3):
            for c in range(3):
                ax.text(c, r, format(data[r, c], fmt), ha="center", va="center",
                        ...
                        fontsize=10)
    title = "PINN Classifier — Confusion Matrices" + (f" ({tag})" if tag else "")
...
def plot_training_curves():
    ...
    axes[0].set(title="PINN — Loss", xlabel="Epoch", ylabel="Loss"); axes[0].legend()
    axes[1].plot(ep, [v*100 for v in h["train_acc"]], label="Train")
    axes[1].plot(ep, [v*100 for v in h["val_acc"]],   label="Val")
    axes[1].set(title="PINN — Accuracy", xlabel="Epoch", ylabel="Accuracy (%)"); axes[1].legend()
    plt.tight_layout()
...
def plot_confidence(labels, preds, probs):
    ...
    ax.set(xlabel="Max Softmax Confidence", ylabel="Count",
           title="PINN — Confidence Distribution"); ax.legend(fontsize=11)
    plt.tight_layout()
...
def plot_ring_examples(model, loader, device, n=9):
    """Visualise predicted ring overlays on sample images."""
    model.eval()
    images_batch, labels_batch = next(iter(loader))
    images_batch = images_batch[:n].to(device)

    with torch.no_grad():
        logits, rings, _ = model(images_batch)
    preds = logits.argmax(1).cpu().numpy()
    imgs  = images_batch.cpu().numpy()
    rings = rings.cpu().numpy()

    fig, axes = plt.subplots(3, 3, figsize=(10, 10))
    for i, ax in enumerate(axes.flat):
        if i >= n: ax.axis("off"); continue
        img_disp = (imgs[i, 0] + 1) / 2          # [-1,1] → [0,1]
        ring_disp = rings[i, 0]

        ax.imshow(img_disp, cmap="gray", vmin=0, vmax=1)
        # Overlay ring template as a coloured contour
        ax.contour(ring_disp, levels=[0.5], colors=["#FF4444"], linewidths=1.5)
        true_lbl = labels_batch[i].item()
        ax.set_title(
            f"True: {CLASS_NAMES[true_lbl]}\nPred: {CLASS_NAMES[preds[i]]}",
            fontsize=8, color="green" if true_lbl == preds[i] else "red")
        ax.axis("off")

    fig.suptitle("PINN — Predicted Einstein Ring Overlays (red contour)", fontsize=12)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, "ring_examples.png")
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
    print(f"Saved: {path}")
...
def main():
    ...
    print(f"PINN (Test VII):           {acc*100:.2f}% std | {acc_tta*100:.2f}% TTA")
