"""Training script for HybridFNOClassifier (Test III / Specific Test IV, v3).

Dataset split
-------------
The raw dataset has 37,500 images (12,500 per class) spread across
pre-existing train/ (10 K/class) and val/ (2.5 K/class) directories.
We merge both directories and re-split 90:10 (stratified per class,
seed=42), giving 11,250 train / 1,250 val per class  →
33,750 train / 3,750 val total — as required by the evaluation guidelines.

Two-phase training
------------------
Phase 1 (20 epochs, backbone frozen):
  Only FNO blocks + head are trained (LR=1e-3).
Phase 2 (80 epochs, all unfrozen, differential LR):
  backbone 1e-4 / FNO+head 5e-4 — preserves pretrained features.
...
Outputs
-------
    weights/best_fno.pth      -- Best validation checkpoint
...
# ── 90:10 split helper ─────────────────────────────────────────────────────────
def get_split_paths(dataset_root: str, val_frac: float = 0.10, seed: int = 42):
    """
    Merge pre-existing train/ and val/ dirs, re-split 90:10 stratified per class.

    Returns
    -------
    train_items, val_items : list of (path, label) tuples
    """
    ...
    for sub, label in LABEL_MAP.items():
        ...
        # Sort by integer stem for determinism (1.npy < 2.npy < ... < 12500.npy)
...
# ── Dataset ────────────────────────────────────────────────────────────────────
def _log_stretch(x: np.ndarray) -> np.ndarray:
    return np.log1p(x * 10.0) / np.log1p(10.0)


class LensingDataset(Dataset):
    """Preloads all images into RAM from a (path, label) item list."""

    def __init__(self, items: list, augment: bool = False) -> None:
        self.augment = augment
        print(f"  Preloading {len(items):,} images into RAM ...", flush=True)
        images, labels = [], []
        ...
        print(f"  Done. Tensor shape: {self.images.shape}", flush=True)
    ...
    def _augment(x: torch.Tensor) -> torch.Tensor:
        ...
        if random.random() > 0.4:
            ...
            x = F.grid_sample(x.unsqueeze(0).float(), grid,
                              mode="bilinear", padding_mode="zeros",
                              align_corners=False).squeeze(0).to(x.dtype)
        if random.random() > 0.5:
            alpha = random.uniform(0.85, 1.15)
            beta  = random.uniform(-0.05, 0.05)
            x = (x * alpha + beta).clamp(-1.0, 1.0)
        if random.random() > 0.5:
        ...
        if random.random() > 0.6:
            ...
            ri = random.randint(0, H - eh)
            rj = random.randint(0, W - ew)
            x = x.clone()
            x[:, ri:ri+eh, rj:rj+ew] = x.mean()
        return x
...
def main():
    ...
    print(f"HybridFNO parameters: {model.count_all_parameters():,}", flush=True)
    ...
    for epoch in range(1, PHASE1 + 1):
        tr_loss, tr_acc = run_epoch(model, train_loader, criterion, opt1, sched1,
                                    device, scaler, use_amp)
        vl_loss, vl_acc = run_epoch(model, val_loader,   criterion, None, None,
                                    device, scaler, use_amp)
        history["train_loss"].append(tr_loss); history["val_loss"].append(vl_loss)
        history["train_acc"].append(tr_acc);   history["val_acc"].append(vl_acc)
        print(f"[P1] Epoch {epoch:3d}/{PHASE1} | "
              f"Train {tr_acc*100:.2f}% | Val {vl_acc*100:.2f}% | "
              f"LR {opt1.param_groups[0]['lr']:.2e}", flush=True)
        if vl_acc > best_val_acc:
            best_val_acc = vl_acc; patience_ctr = 0
            torch.save(model.state_dict(), os.path.join(WEIGHTS_DIR, "best_fno.pth"))
            print(f"  -> Saved (val_acc={best_val_acc*100:.2f}%)", flush=True)
        else:
            patience_ctr += 1
            if patience_ctr >= PATIENCE1:
                print(f"  Phase-1 early stop at epoch {epoch}", flush=True); break

    print(f"Phase 1 done. Best val acc: {best_val_acc*100:.2f}%", flush=True)

    # ── Phase 2: all unfrozen, differential LR ────────────────────────────────
    PHASE2, PATIENCE2 = 80, 20
    ...
    for epoch in range(1, PHASE2 + 1):
        tr_loss, tr_acc = run_epoch(model, train_loader, criterion, opt2, sched2,
                                    device, scaler, use_amp)
        vl_loss, vl_acc = run_epoch(model, val_loader,   criterion, None, None,
                                    device, scaler, use_amp)
        history["train_loss"].append(tr_loss); history["val_loss"].append(vl_loss)
        history["train_acc"].append(tr_acc);   history["val_acc"].append(vl_acc)
        print(f"[P2] Epoch {epoch:3d}/{PHASE2} | "
              f"Train {tr_acc*100:.2f}% | Val {vl_acc*100:.2f}% | "
              f"LR {opt2.param_groups[0]['lr']:.2e}", flush=True)
        if vl_acc > best_val_acc:
            best_val_acc = vl_acc; patience_ctr = 0
            torch.save(model.state_dict(), os.path.join(WEIGHTS_DIR, "best_fno.pth"))
            print(f"  -> Saved (val_acc={best_val_acc*100:.2f}%)", flush=True)
            ...
            patience_ctr += 1
            if patience_ctr >= PATIENCE2:
                print(f"  Phase-2 early stop at epoch {epoch}", flush=True); break
